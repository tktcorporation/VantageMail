...
>() {
  /** 手動で実装を渡す場合（テスト等） */
  static layer = (impl: SessionServiceImpl) => Layer.succeed(SessionService, impl);
  ...
  static live = (sessionSecret: string) =>
    Layer.succeed(SessionService, {
      get: () =>
        Effect.tryPromise({
          try: async () => {
            const session = await getSession<AppSessionData>(getSessionConfig(sessionSecret));
            return session.data;
          },
      ...
      update: (fn) =>
        Effect.tryPromise({
          try: () => updateSession<AppSessionData>(getSessionConfig(sessionSecret), fn),
          catch: (e) => new SessionError({ reason: String(e) }),
        }),
      ...
      clear: () =>
        Effect.tryPromise({
          try: () =>
            updateSession<AppSessionData>(getSessionConfig(sessionSecret), () => ({
              userId: undefined,
              dek: undefined,
