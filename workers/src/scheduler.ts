  ...
  Effect.gen(function* () {
    ...
    if (body.accountEmail) {
      ...
      if (existing) {
        ...
        yield* Effect.tryPromise({
          try: () =>
            env.SCHEDULED_JOBS!.put(
              idxKey,
              JSON.stringify(jobIds.filter((k) => k !== body.key)),
            ),
          catch: (e) => new Error(`KV put index failed: ${e}`),
        })
      }
...
export async function processScheduledJobs(
  ...
  const program = Effect.gen(function* () {
    ...
    for (const key of jobKeys.keys) {
      ...
      ctx.waitUntil(
        Effect.runPromise(processJob(job, env)).finally(() =>
    ...
    for (const key of accounts.keys) {
      ...
      yield* Effect.tryPromise({
        try: async () => {
          ...
          if (!tokenRes.ok) {
            console.error(
              `Token refresh failed for ${key.name}: ${tokenRes.status}`,
            )
            return
          }
